...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        map_path = os.path.abspath(os.path.join('src/icra_ws/src', 'csv_data'))
        ...
        self.ref_speed = csv_data[:, 5] * 1.17 # max speed - sim is 10m/s

        self.visualization_init()

        # params for levine 2nd - real
        #velocity = self.ref_speed[self.closest_index]
        #L = 2 * velocity - 3.5
    ...
    def build_control_functions(self, speed, v_min, v_max):
        # Lookahead distance mapping: v_min -> 1.5, v_max -> 2.3
        d_min = 1.0
        d_max = 2.5
        ...
        # Steering gain mapping: v_min -> 0.5, v_max -> 0.2
        gain_max = 0.7
        gain_min = 0.65
    ...
    def pose_callback(self, pose_msg):
        ...
        print("position = {}, steering = {}, speed = {}".format(round(self.currX, 4), round(self.drive_msg.drive.steering_angle, 2), round(self.drive_msg.drive.speed, 2)))
